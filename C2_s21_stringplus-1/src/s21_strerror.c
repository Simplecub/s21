...
char error_unknown[101] = {0};
char *s21_strerror(int errnum) {
  char *ptr = NULL;
  if (errnum > MAX || errnum < 0) {
    char number[30] = {0};
    int_to_arr(errnum, number);
#ifdef __APPLE__
    s21_memset(error_unknown, 0, 100);
    s21_strncat(error_unknown, ERROR, 100);
#endif
#ifdef __linux__
    s21_memset(error_unknown, 0, 100);
    s21_strncat(error_unknown, ERROR, 100);
#endif
    s21_strncat(error_unknown, number, 100);
    ptr = (char *)error_unknown;
  } else {
