...
void print_overlay() {
  ...
  mvprintw(1, BOARD_W + 5, "HIGH SCORE:");
  mvprintw(3, BOARD_W + 5, "SCORE:");
  mvprintw(5, BOARD_W + 5, "LEVEL:");
  mvprintw(7, BOARD_W + 5, "SPEED:");
  ...
  mvprintw(BOARD_H / 2, (BOARD_W - INTRO_MESSAGE_LEN1) / 2 + 1, INTRO_MESSAGE1);
  mvprintw(BOARD_H / 2 + 1, (BOARD_W - INTRO_MESSAGE_LEN2) / 2 + 1,
           INTRO_MESSAGE2);
}
...
void game_loop(void) {
  ...
  statsInit(&params);
  print_overlay_data(&stats_game);
  print_next(params.stats_game->next);
    ...
    if (timer >= 1.5 - params.stats_game->speed * SPEED_MUL) {
      if (params.status == GAME && !params.stats_game->pause) {
        *params.stats_game = updateCurrentState();
        clear();
        if (params.status == START) print_overlay();
